...
qbase    = 1.0135023;                                 @--base price of claims-------@

qgrid=zeros(10,1);
for a(1,10,1);
    qgrid[a]=qbase-0.012+0.0024*(a-1);
endfor;
/*stop;*/
...
for j(1,Nk-1,1);
    ...
    for i(1,Np,1);
        ...
        elseif agrid[i] >= als[Np,j];
            ...
            al[i,j+1] = LIP(als[.,j],agrid,agrid[i]);      
        endif;
        ...
        elseif agrid[i] >= als[Np,j];
            ...
            ah[i,j+1] = LIP(ahs[.,j],agrid,agrid[i]);      
        endif;
    endfor;
    /* 
...
for i(1,Nk-1,1);
    for j(1,Md,1);
        if mgrid[j] < gl[1];
        m0 = LIP(mgrid,F0[.,i],LIP(gl,agrid,mgrid[j]));
        m1 = 0;
        ...
        m0 = LIP(mgrid,F0[.,i],LIP(gl,agrid,mgrid[j]));
        m1 = LIP(mgrid,F1[.,i],LIP(gh,agrid,mgrid[j]));
        F0[j,i+1] = p00*m0+p10*m1;
        F1[j,i+1] = p01*m0+p11*m1; 
...
_plegstr= " ED\0"\
          " beta\0";
/*scale(0,0|12.5);*/
xlabel("Market disequilibrium");
ylabel("Price of claims and discount factor");

title("Price of claims and market disequilibrium, based on Salas");

xy(EXDAgrid,invqgrid~discount);
